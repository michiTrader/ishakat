...
func TestRenderReasoningPreviewOffShowsNothing(t *testing.T) {
	...
	got := renderReasoningPreview(styles, "the model thinks step by step about this", "off", 40)
	if got != "" {
...
func TestRenderReasoningPreviewEmptyReasoningShowsNothing(t *testing.T) {
	...
	for _, mode := range []string{"off", "collapsed", "full", "", "bogus"} {
		if got := renderReasoningPreview(styles, "   ", mode, 40); got != "" {
			t.Errorf("mode=%q with blank reasoning must render nothing, got %q", mode, got)
		}
	}
}
...
func TestRenderReasoningPreviewCollapsedTruncatesWithEllipsis(t *testing.T) {
	...
	got := renderReasoningPreview(styles, long, "collapsed", 20)
	plain := stripANSI(got)
...
func TestRenderReasoningPreviewCollapsedShortStaysWhole(t *testing.T) {
	...
	got := renderReasoningPreview(styles, short, "collapsed", 40)
	plain := stripANSI(got)
...
func TestRenderReasoningPreviewFullShowsEverything(t *testing.T) {
	...
	got := renderReasoningPreview(styles, long, "full", 20)
	plain := stripANSI(got)
	if strings.Contains(plain, "…") {
...
func TestRenderReasoningPreviewUsesDimStyle(t *testing.T) {
	...
	got := renderReasoningPreview(styles, "brief thought", "full", 40)
