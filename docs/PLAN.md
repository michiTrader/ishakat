...
| **30** ✅ | **Trust-once** (§21.4 layer 2) — **closed**, §17 2026-08-16 | ✅ `TestResolveProjectTrustSecondRunAsksNothing`/`TestFileTrustStoreSavePersistsRecord`: a project with a saved `trust.json` record opens straight to `ModeChat` with `FooterState.Autonomy` already set, never `ModeTrust`; `Esc` on the dialog resolves to "Ask before changes" (agile), never the recommended Auto option; `[autonomy].remember = false` asks every run |
...
| **30** ✅ | Trust-once (§21.4 layer 2) + autonomy in the status line — **closed**, §17 2026-08-16 | ✅ second run in a known project asks nothing (`TestResolveProjectTrustSecondRunAsksNothing`, `TestNewRootOpensModeTrustWhenNeedsTrust`) |
