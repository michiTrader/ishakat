...
| 32 | **`ask_user`, `/permissions`, phase display** (§21.1, §21.16) — 🟡 **parts 1-4 landed (`ask_user` tool, wiring, TUI bridge, serve bridge), `/permissions`/phase display still open** | ⬜ `ask_user` ships as the ninth core tool under §19.1's one documented exception (settled 2026-08-15, *not* as a `dispatch` mode): always present, always `safe`, never denyable. The human gains one place to audit rules and invariants, and the status line gains `auto·exec`/`auto·wait 22s`. **No `shift+tab`** — the shortcut is evaluated only after `/permissions` is used on a real Termux device |
