...
| 31 | **Missions and the constraint compiler** (§21.6, §21.11) — 🟡 **in progress**, §17 2026-08-16 (part 1), 2026-08-16 (part 2), 2026-08-16 (part 3) | 🟡 "no Playwright" compiles to a deny rule the Guard enforces (`internal/mission.Compile` + `Guard.AddMissionRules`/`hardDeny`, `TestGuardMissionRuleSurvivesYolo`) and sub-agents inherit it by pointer identity (`TestGuardMissionRuleInheritedBySubAgentGuard`), but cannot widen it. ✅ The confirmation dialog (`ModeMission`, `internal/tui/mission.go`) and wiring a stated goal's compiled mission into a real running session (`tui.Options.MissionGuard`, `internal/app/app.go`'s `missionGuardOrNil`) landed in part 2. ✅ A dispatched sub-agent's own stated goal (`toolTarget`'s new `task` case) and the inherited mission constraint (`no bash(**playwright**)`, via `Guard.MissionRules()`'s own long-anticipated caller in `toolactivity.go`) now render on the transcript line for `dispatch`, landed in part 3. ⬜ Still missing: the tool-scope proposal dialog ("Tools for this mission") and the full §21.11 parallel fan-out/swarm display (the engine has no concurrent multi-sub-agent concept yet — only one serial `dispatch` call per turn) |
...
| **31** 🟡 | Missions + constraint compiler (§21.6) + inheritance (§21.11) — **in progress**, §17 2026-08-16 (part 1 + part 2 + part 3) | ✅ "no Playwright" produces a deny rule a sub-agent cannot widen (`TestGuardMissionRuleDeniesBashCommand`/`TestGuardMissionRuleInheritedBySubAgentGuard`) ✅ confirmation dialog (`ModeMission`) + app wiring (`MissionGuard`) landed part 2 ✅ a dispatched sub-agent's stated goal and the inherited `no bash(**playwright**)`-shaped constraint line now render on the transcript (`TestToolActivityLinesShowsInheritedMissionRulesOnDispatch`, `TestFinishAgentTurnShowsInheritedMissionRulesOnADispatchLine`) landed part 3 ⬜ tool-scope dialog + full §21.11 parallel fan-out/swarm display still not done |
