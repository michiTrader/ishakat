...
| 31 | **Missions and the constraint compiler** (§21.6, §21.11) — 🟡 **in progress**, §17 2026-08-16 (part 1), 2026-08-16 (part 2) | 🟡 "no Playwright" compiles to a deny rule the Guard enforces (`internal/mission.Compile` + `Guard.AddMissionRules`/`hardDeny`, `TestGuardMissionRuleSurvivesYolo`) and sub-agents inherit it by pointer identity (`TestGuardMissionRuleInheritedBySubAgentGuard`), but cannot widen it. ✅ The confirmation dialog (`ModeMission`, `internal/tui/mission.go`) and wiring a stated goal's compiled mission into a real running session (`tui.Options.MissionGuard`, `internal/app/app.go`'s `missionGuardOrNil`) landed in part 2. ⬜ Still missing: the tool-scope proposal dialog and the sub-agent `no browser · no network` status line |
