...
| 32 | **`ask_user`, `/permissions`, phase display** (§21.1, §21.16) — 🟡 **parts 1-7 landed (`ask_user` tool, wiring, TUI bridge, serve bridge, `/permissions` read side, `/permissions` autonomy write, recently-denied list); phase display still open** | ⬜ `ask_user` ships as the ninth core tool under §19.1's one documented exception (settled 2026-08-15, *not* as a `dispatch` mode): always present, always `safe`, never denyable. The human gains one place to audit rules and invariants (`/permissions`, parts 5-7, §17 2026-08-17), can change autonomy from that same command (§21.16 decision 4: "the whole interface for reading and changing autonomy"), and can now see what the session has refused lately (§21.13's own acceptance-narrative item 10, part 7) — but the status line still does not gain `auto·exec`/`auto·wait 22s` — no `Phase` concept exists yet. **No `shift+tab`** — the shortcut is evaluated only after `/permissions` is used on a real Termux device |
