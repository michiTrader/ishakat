...
**Status:** Phase 1, Phase 2.5 and **Phase 3 closed** (Steps 0–25 · live `/theme` + `ctrl+t` overlay + colour degradation + input box/footer/autocomplete + syntax highlighting/Chroma + Markdown/Glamour) · **Phase 4 (robustness) in progress** — retries/backoff, timeouts, readable errors, offline mode, automatic `fallback_model` switching and both the Anthropic and Gemini native dialect adapters (including Gemini's own `httptest` test suite) are closed (see §17 2026-08-13 `checkFallback`, §17 2026-08-14 `internal/provider/anthropic`, and §17 2026-08-14 `internal/provider/gemini` + its test suite); only the full security-review pass remains open (see §11 Phase 4)
