...
| 2026-08-13 | Fase 3, incremento 1 · overlay `ctrl+t` (ThemePicker) — closed, cierra el único item pendiente de PR #115 | El item explicitamente diferido en la entrada anterior de este mismo Bitácora: §9.7's wireframe nombra `/theme [nombre]` y `ctrl+t` juntos como los dos caminos de acceso a la misma capacidad, y `keys.go`'s `ThemePicker: "ctrl+t"` existia sin handler desde el Paso 13 — `theme.go`'s propio package comment ya lo flagueaba como la pieza que faltaba. **Fix, un commit.** Nuevo `internal/tui/themepicker.go`: `themePickerState` (lista plana sin agrupación — los temas no tienen split proveedor/tier como el catálogo de modelos, así que sigue la forma más simple de `resumeMenu`/`confirmDialog` en vez de la de `Picker`), con su propio `moveSel`/`selected`. `openThemePicker` lista exactamente lo que `listThemes` (`theme.go`) imprimiría para un `/theme` sin argumento — el mismo `theme.Available(m.themesDir)` más `sortedThemeNames` — con el cursor arrancando en el tema activo. Enter aplica la elección llamando `switchTheme` (`theme.go`) directamente: el overlay es una segunda puerta a la misma implementación — mismo texto de confirmación, misma persistencia vía `ThemeStore`, nunca un camino paralelo que pudiera desincronizarse de `/theme [nombre]`. `internal/tui/root.go` gana `ModeThemePicker` en el enum `Mode`, el campo `Root.themePicker`, un `case m.keys.ThemePicker` en `handleGlobalKey` con el mismo gating ModeChat-only que `ModelPicker`/`CopyLast` (un segundo `ctrl+t` se traga en vez de reabrir el overlay), y su `case` en `updateDispatch`. `internal/tui/view.go` gana su `case` en `renderRaw`; `cursorFor` ya apagaba el cursor fuera de `ModeChat` sin necesitar cambios. 7 tests nuevos en `internal/tui/themepicker_internal_test.go`: `ctrl+t` abre el overlay desde `ModeChat` listando los temas; se traga fuera de `ModeChat`; `esc` cancela sin cambiar el tema; up/down envuelve (wrap) igual que `Picker`/`resumeMenu`; enter aplica el tema por el mismo camino que `/theme [nombre]` (mismo texto de confirmación, misma mutación de `styles.Theme.Name`, verificado literalmente contra `TestSlashThemeWithNameSwitchesLive`'s propio contrato); enter persiste vía `ThemeStore` igual que `/theme` (`TestSlashThemePersistsViaThemeStore`'s mismo patrón); enter con la lista vacía es un no-op. **Verificación:** `gofmt -l .` limpio, `go build ./...` OK, `go vet ./...` limpio, `go test ./...` todo verde (22 paquetes). Las cinco pruebas de frontera arquitectónica de `internal/arch_test.go` (`TestTUINoImportaHTTP` incluida) re-ejecutadas explícitamente y pasan. **PR #115 ("Fase 3, incremento 1: /theme [nombre]") queda ahora sin items pendientes** — §9.7's wireframe (`/theme [nombre]` + `ctrl+t`) está completamente respondido. Fase 3's restantes items de prosa (Markdown/Glamour, resaltado de sintaxis/Chroma, las dos animaciones de "idea visual de producto") siguen sin empezar, fuera del alcance de este incremento. |
| 2026-08-13 | Fase 3, incremento 2 · bloques de código resaltados (Chroma) — closed | §11's Fase 3 named this as one of three remaining items (line ~1460, "bloques de código resaltados — Chroma entra aquí"), preferred over Markdown/Glamour (§6.4's own budget note: Glamour pulls in goldmark, bluemonday, CSS parsers and a duplicate lipgloss v1 major version, vs. Chroma's single extra dependency `github.com/dlclark/regexp2/v2`) and over the two "idea visual" animations (no urgency signal, and this feature already had scaffolding sitting unused since Step 3: `theme.Theme.Syntax map[string]RGB` parsed from every theme's TOML but never read by anything in `internal/tui`, `config.UI.Syntax bool` defaulting to `true` in `defaults.toml` but never consumed, and `ascua.toml`'s own `[syntax]` table with `keyword`/`string`/`comment`/`number` colours already defined). `chat.go`'s own `renderTranscriptLine` doc comment had literally said "Markdown is still deferred" since Step 3 — the exact marker for where this needed to be wired in. **Fix, one commit.** New `internal/tui/codeblock.go`: `splitCodeSegments` walks a message body line by line, cutting it into prose/code runs on triple-backtick fences (an unclosed fence at end-of-text still reads as code — required since the dummy echo test engine streams text back in small chunks, so a real message's fence necessarily arrives split across several render ticks); `renderMessageBody` is `renderTranscriptLine`'s text half, now fence-aware, falling back to plain `wrapText` byte-for-byte when there is no fence at all; `renderCodeBlock` draws §9.3's own wireframe convention — a left rail (`│`) instead of a full box, "a 40 columnas una caja roba 4 columnas útiles" — with an optional language name line; `highlightSource` tokenises via `github.com/alecthomas/chroma/v2`'s `lexers.Get`/`Tokenise` (nil lexer or tokenise error both fall back to plain, undecorated text, never a panic) and `syntaxStyleFor` maps each token's `chroma.TokenType` onto exactly the four keys the theme's `[syntax]` table defines (`keyword`/`string`/`comment`/`number` via `InCategory`/`InSubCategory`), returning no style at all for every other token kind or when `styles.Cap == theme.CapNone`. Wired via a new `Root.cfgSyntax bool` field (`o.Cfg == nil || o.Cfg.UI.Syntax`, mirroring `cfgBanner`'s own pattern) threaded through `chat.go`'s `renderTranscriptLine`/`renderLiveTurn`/`commitEntryCmd` and `view.go`'s two call sites — `cfgSyntax == false` still draws the rail (a layout choice, not a colour one) and only skips Chroma's own per-token colouring, the same distinction `theme.CapNone` already draws for a terminal that cannot show colour regardless of what the config says. 13 new tests in `internal/tui/codeblock_internal_test.go`: fence extraction (identity with no fence, a 3-segment prose/code/prose split matching §9.3's own SQL example, an unclosed fence still reading as code, a fence with no language named), rail rendering on every line including the language row, known-token-kind colouring (checked via lipgloss's actual truecolor escape format `"38;2;R;G;Bm"`, not `RGB.Hex()`'s `"#rrggbb"` string — an early assertion bug, fixed), unknown/empty-language plain fallback, `theme.CapNone` never colouring a single token, the no-fence fast path matching `wrapText` exactly, `highlightCode=false` still drawing the rail while emitting zero per-token keyword-colour escapes (not zero escapes overall — the rail/language line legitimately keep using `styles.Border`/`styles.Dim`, a second early assertion bug, fixed), and one full `newVisibleRoot()`/`playTurn()` integration test sending a fenced Go snippet through the real echo-engine streaming path, confirming the rail appears on screen, every character of the code survives `stripANSI`, and no fence marker (`` ``` ``) leaks onto the rendered frame. **Verificación:** `gofmt -l .` limpio, `go build ./...` OK (after a required second `go mod tidy` — the first ran before `codeblock.go`'s own import existed and pruned the just-added dependency back out), `go vet ./...` limpio, `go test ./...` todo verde (23 paquetes). Las cinco pruebas de frontera arquitectónica de `internal/arch_test.go` re-ejecutadas explícitamente y pasan, confirmando que Chroma's full dependency closure (only `github.com/dlclark/regexp2/v2`) does not leak `net/http` or cross any of §6.1's five forbidden boundaries. Fase 3's restantes items de prosa (Markdown/Glamour, las dos animaciones de "idea visual de producto") siguen sin empezar, fuera del alcance de este incremento. |
