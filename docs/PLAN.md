...
| 31 | **Missions and the constraint compiler** (§21.6, §21.11) — 🟡 **in progress**, §17 2026-08-16 | 🟡 "no Playwright" compiles to a deny rule the Guard enforces (`internal/mission.Compile` + `Guard.AddMissionRules`/`hardDeny`, `TestGuardMissionRuleSurvivesYolo`) and sub-agents inherit it by pointer identity (`TestGuardMissionRuleInheritedBySubAgentGuard`), but cannot widen it. ⬜ Still missing: the confirmation dialog itself, the tool-scope proposal dialog, and wiring a stated goal's compiled mission into a real running session |
...
| **31** 🟡 | Missions + constraint compiler (§21.6) + inheritance (§21.11) — **in progress**, §17 2026-08-16 | ✅ "no Playwright" produces a deny rule a sub-agent cannot widen (`TestGuardMissionRuleDeniesBashCommand`/`TestGuardMissionRuleInheritedBySubAgentGuard`) ⬜ confirmation dialog + app wiring not yet done |
