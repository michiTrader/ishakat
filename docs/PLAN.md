...
| 2026-08-06 | Session handoff · P2 items not started | The three P2 items from the original audit's ranked list remain open, deliberately not started this session to leave a clean, fully-tested stopping point rather than several half-finished changes: **(1)** translate `ErrNoAPIKey` (`internal/provider/provider.go`) and the other Spanish strings the audit flagged (`checkPerms`'s `"permisos inseguros..."` in `expand.go`, `config`'s `"clave ignorada: "` in `load.go`, `app.go`'s `"✗ Error de configuración"`/`"✗ Error ejecutando la interfaz"`) to English per AGENTS.md's language policy — note `openai_test.go:434-435` asserts on `ErrNoAPIKey`'s Go identity via `errors.Is`, not its string, so this is a pure string change with no call-site risk. **(2)** `provider add` with no arguments today prints usage and exits 2 (`cmdProviderAdd`'s `len(args) == 0` guard, `cmd/ishakat/provider.go`); the audit's proposed fix is a short picker over `config.ProviderPresets()` (5 entries) so `ishakat provider add` alone still completes the flow. **(3)** `config init` (referenced in `cmd/ishakat/main.go`'s usage text) currently always writes the full annotated `config.example.toml`; the audit's proposed fix is a minimal skeleton by default (schema, empty `[app]`, a comment pointing at `provider add`) with `--full` opting into today's behaviour — note `internal/config/config_test.go` has a drift-guard test asserting `config.ExampleTOML` matches the on-disk `config.example.toml` byte-for-byte, which any change to that embed must keep passing. |
