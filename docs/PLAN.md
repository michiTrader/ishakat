...
| 24 | **`/login`** (OAuth device flow + API-key wizard) — **API-key wizard CLOSED** (pulled forward, see §17 2026-08-04..06); **OAuth device flow CLOSED for the CLI**, see §17 2026-08-12; **`/login` slash-command row CLOSED**, see §17 2026-08-12 (same date, second entry); **TUI-side interactive wizard not started** | ✅ `ishakat provider add\|list\|remove` (API key). ✅ `ishakat login <provider>` drives `internal/oauth`'s RFC 8628 client — provider-agnostic by design, since none of the five built-in presets has a ToS-clean device flow to enable (see `cmd/ishakat/login.go`'s own doc comment). ✅ `/login` has a row in `internal/slash/slash.go`'s `Commands` table (`/help` and the autocomplete dropdown both list it) and a `KindUnimplemented` case in `slashrun.go` pointing at the CLI command, following the exact same "honest pending" pattern `/config`/`/debug` already established. ⬜ No TUI-side interactive wizard yet — driving `internal/oauth` from inside a running session needs the HTTP-driving half injected via a factory, since `internal/tui` cannot import `net/http` |
...
| `/login` | autenticar via OAuth device flow | ⬜ paso 24 · fila en la tabla desde 2026-08-12; hoy `KindUnimplemented` apunta a `ishakat login <proveedor>` (el wizard real, ya cerrado a nivel binario) en vez de un no-op silencioso — el wizard dentro de la TUI misma no está escrito |
| `/exit` | salir | ✅ |
