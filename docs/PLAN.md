...
| 13bis | **Distribución: `curl \| sh` + GitHub Actions** (adelantado desde Fase 5 · **CERRADA**) | ⬜ `workflows` scope unblocked, `ci.yml`+`release.yml` live at `.github/workflows/` and `ci.yml` green on `main` (run 31137816770) — pending: push tag `v0.1.0` and watch `release.yml`'s `verify-android-doctor` job pass for real before this row flips to done. See §17 2026-08-07. |
...
| 2026-08-07 | Step 13bis · `workflows` scope blocker resolved by the user; `ci.yml`/`release.yml` live and CI green — still not the closing entry | The user ran the two-command `git mv` documented in `docs/dist-workflows-staging/README.md` from a credential that does carry the `workflows` scope and pushed directly to `main` (commit `7b5e4ff`, `ci: activate staged GitHub Actions workflows`) — no code content changed, both files moved byte-for-byte from `docs/dist-workflows-staging/` to `.github/workflows/`. Confirmed this session, from a clean `git fetch`: `.github/workflows/ci.yml` and `release.yml` exist on `origin/main`, and `gh run list` shows CI's first real run on the new path (`31137816770`, triggered by that same push) completed both jobs green — `build · vet · fmt · test` in 39s, `race detector` in 53s, only a non-blocking Node 20 deprecation annotation from `actions/checkout@v4`/`actions/setup-go@v5`. Re-ran the full local check suite independently of CI to be sure nothing in the working tree drifted from what CI saw: `go build ./...`, `go vet ./...`, `gofmt -l cmd internal` (empty), `go test ./...` all green, 16/16 packages, Go 1.26.5. Removed `docs/dist-workflows-staging/` in this same session — it was a holding pen for a blocker that no longer exists, and its own `README.md` said explicitly that nothing should treat it as a real location once unblocked. **Why this entry does not close the row:** `ci.yml` runs on every push and just proved itself; `release.yml` only fires on a `v*` tag, and no tag exists yet in this repo (`git tag -l` and `gh release list` both empty) — so the actual closing criterion for 13bis, `verify-android-doctor` passing DNS+HTTPS inside a real booted Android emulator, has never run once. The row stays open until a `v0.1.0` tag is pushed and that job is watched succeed. |
