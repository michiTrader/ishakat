...
| 32 | **`ask_user`, `/permissions`, phase display** (§21.1, §21.16) — 🟡 **parts 1-8 landed (`ask_user` tool, wiring, TUI bridge, serve bridge, `/permissions` read side, `/permissions` autonomy write, recently-denied list, `exec`/`ask` phase display); `wait` phase (OnWait→TUI bridge) still open** | ⬜ `ask_user` ships as the ninth core tool under §19.1's one documented exception (settled 2026-08-15, *not* as a `dispatch` mode): always present, always `safe`, never denyable. The human gains one place to audit rules and invariants (`/permissions`, parts 5-7, §17 2026-08-17), can change autonomy from that same command (§21.16 decision 4: "the whole interface for reading and changing autonomy"), and can now see what the session has refused lately (§21.13's own acceptance-narrative item 10, part 7) — and the status line now draws `auto·exec`/`auto·ask` (part 8, §17 2026-08-17), covering acceptance-narrative item 3 ("`auto·exec`. No dialog."). **Still open**: `auto·wait 22s` (item 6 — no `OnWait`→TUI bridge exists yet, engine.AgentOptions.OnWait is wired only in the headless path) and `plan`/`check` (items 5/`auto·plan` — no real plan/exec split exists in the running loop, §21.14's own Step 31 closing note). **No `shift+tab`** — the shortcut is evaluated only after `/permissions` is used on a real Termux device |
